...
from fedml_api.standalone.domain_generalization.datasets.utils.public_dataset import PublicDataset
from argparse import Namespace

def get_all_models():
    return [model.split('.')[0] for model in os.listdir('../../../fedml_api/standalone/domain_generalization/datasets')
            if not model.find('__') > -1 and 'py' in model]

Priv_NAMES = {}
for model in get_all_models():
    mod = importlib.import_module('fedml_api.standalone.domain_generalization.datasets.' + model)
